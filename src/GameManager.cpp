...
GameManager::GameManager() : currentDay(1), gameRunning(true) {
    ...
    player->getPlayersInventory()->addItem(Carrotitem, 5);
    player->getPlayersInventory()->addItem(Potatoitem, 3);
}
...
void GameManager::initializeGame() {
    ...
    if (COLS < 111 || LINES < 43) {
        endwin();
        std::cout << "Terminal too small! Need at least 111x43" << std::endl;
        gameRunning = false;
        return;
    }
    
    // Create windows
    gameWin = newwin(LINES, COLS, 0, 0);
    farmWin = newwin(FARM_HEIGHT + 2, FARM_WIDTH + 2, 2, 2);
    inventoryWin = newwin(LINES - 4, INVENTORY_WIDTH, 2, COLS - INVENTORY_WIDTH - 2);
    infoWin = newwin(INFO_HEIGHT, FARM_WIDTH + 2, FARM_HEIGHT + 4, 2);
